...
;+
; The event handler for the top-level base in plotinteractive. Handles
; resize events.
;
...
pro plotinteractive_resize, event
    ...
    ; To properly resize the draw widget, the geometry of the bases
    ; surrounding it must be determined.
    ...
    ; Determine a new xsize for the draw widget.
    newx = event.x - 2*tlbg.xpad

    ; Determine a new ysize for the draw widget.
...
;+
; This program displays a resizeable window for displaying plots. Graphical
; interfaces are provided to a few of the keywords for the IDL built-in
; routine PLOT.
;
; <center><img src="plotinteractive.png" alt="plotinteractive" /></center><br>
;
...
pro plotinteractive, data
    ...
    ; Create the top-level base with resize events activated.
    ...
    state = {data:data, win_id:win_id, linestyle:0, psym:0, $
        connect:1, status:status, controlbase:controlbase, draw:draw}
