...
;-
pro polyview_resize_event, event
    compile_opt idl2
...
;===========================================================================
;+
; A view of a polygonal mesh, in Object Graphics.
;
; @requires IDL 6.1
...
pro polyview
    ...
    tlb = widget_base( $
                         title="Yessiree, it's a sea shell.", $
                         /tlb_size_events)
    draw = widget_draw(tlb, $
                       ...
                       /motion_events, $
                       /expose_events, $
                       event_pro='polyview_draw_event')
    widget_control, tlb, /realize

    ;; Get geometry info for the top-level base.
    geom = widget_info(tlb, /geometry)

    ;; Restore the poly data.
    file = filepath('seashell.dat', $
    ...
    op = obj_new('idlgrpolygon', data=v0, polygons=p0, color=[0,0,200])
    om = obj_new('idlgrmodel')
    om->add, op
    ol = obj_new('idlgrlight', type=1, location=[-maxv0,maxv0,maxv0]*1.5)
    olm = obj_new('idlgrmodel')
    olm->add, ol
    ov = obj_new('idlgrview', color=intarr(3)+192S)
    ov->add, om
    ov->add, olm
    ...
    xmanager, 'polyview', tlb, $
        /no_block, $
        cleanup='polyview_cleanup', $
        event_handler='polyview_resize_event'
end
