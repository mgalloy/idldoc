...
; @param value {in}{required}{type=same of the array_list} the value to change the
;        given item to
;-
...
;+
; Free resources of the array_list, including the resources of the elements
; of the array_list if the CLEAN keyword is set.
;
; @keyword clean {in}{optional}{type=boolean} set to cleanup the elements of
;          the array_list as well (if they are objects)
;-
pro array_list::cleanup, clean=clean
    compile_opt idl2
    ;on_error, 2
    ...
    if (keyword_set(clean) && (self.type eq 11)) then begin
        ind = where(obj_valid(*self.data), count)
        for i = 0L, count - 1L do begin
            obj_destroy, (*self.data)[ind[i]]
        endfor
    endif

    if (keyword_set(clean) && (self.type eq 10)) then begin
        heap_free, *self.data
    endif
