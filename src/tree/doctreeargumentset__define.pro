...
;-
function doctreeargumentset::getVariable, name, found=found
  ...
  case name of
    ...
    'index_url': begin
        n_args = self.arguments->count()
        urls = strarr(n_args)
        for a = 0L, n_args - 1L do begin
          arg = self.arguments->get(position=a)
          urls[a] = arg->getVariable('index_url')
        endfor
      end
...
;-
pro doctreeargumentset::getProperty, routine=routine, $
                                     name=name, $
                                     is_first=isFirst, $
                                     is_last=isLast, $
...
; Set properties of the argument.
;-
pro doctreeargumentset::setProperty, is_keyword=isKeyword, $
  ...
  for a = 0L, self.arguments->count() - 1L do begin
    arg = self.arguments->get(position=a)
    arg->setProperty, is_keyword=isKeyword, $
                      is_first=isFirst, is_last=isLast, $
                      is_optional=isOptional, $
                      is_required=isRequired, $
                      is_input=isInput, $
...
;-
function doctreeargumentset::isVisible
  compile_opt strictarr, hidden
...
;-
pro doctreeargumentset::fillLinks
...
;-
function doctreeargumentset::lookupName, name, exclude=exclude
  compile_opt strictarr

  for a = 0L, self.arguments->count() - 1L do begin
    arg = self.arguments->get(position=a)
    url = arg->lookup_name(name, exclude=self.routine)
...
;-
pro doctreeargumentset::cleanup
  compile_opt strictarr, hidden

  obj_destroy, self.firstline
  obj_destroy, self.comments
  obj_destroy, self.arguments
end
...
;-
function doctreeargumentset::init, routine, name=name, is_keyword=isKeyword, $
                                   system=system
  compile_opt strictarr, hidden
  on_error, 2

  if (n_elements(name) eq 0) then message, 'NAME required for argument set'
  ...
  self.arguments = obj_new('MGcoArrayList', type=11, block_size=3)

  names = strtrim(strsplit(self.name, ',', /extract, count=n_names), 2)
  for n = 0L, n_names - 1L do begin
    self.arguments->add, obj_new('DOCtreeArgumentSet', $
                                 self.routine, $
                                 name=names[n], $
...
;     `DOCtreeRoutine` object that contains this argument
;   arguments
;     `MGcoArrayList` of individual arguments
;   name
...
;     text node hierarchy
;-
pro doctreeargumentset__define
  compile_opt strictarr, hidden

  define = { DOCtreeArgumentSet, $
             system: obj_new(), $
             routine: obj_new(), $
             arguments: obj_new(), $
             name: '', $
